...
rustPlatform.buildRustPackage (final: {
  ...
  postInstall = lib.optionalString stdenv.isDarwin ''
    mkdir -p "$out/bin"
    ln -sf "$out/Applications/NIMARA.app/Contents/MacOS/nimara" "$out/bin/nimara"
  '';

  # Add missing runtime dependency
  preFixup = lib.optionalString stdenv.isLinux ''
    patchelf --add-needed libayatana-appindicator3.so.1 $out/bin/nimara
  '';
  ...
  meta = {
    ...
    '';
    homepage = "https://github.com/moeru-ai/nimara";
    changelog = "https://github.com/moeru-ai/nimara/releases/tag/v${final.version}";
    # While nimara itself is licensed under MIT, it uses the nonfree Cubism SDK. Whether it's
