---
title: 桌面版上手指南
description: 如何使用桌面版的 Project NIMARA
---
...
下载完成后，你可以从任何位置启动 NIMARA。你将看到用户界面由两部分组成：
...
更改 NIMARA 的主题颜色，或切换到其他模型，比如 Live2D（2D 模型）或 VRM（3D 模型，就像是 Grok Companion 那样）。
