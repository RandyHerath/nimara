...
async function main() {
  ...
  // Connect nimara server
  const nimaraClient = new Client({
    name: config.nimara.clientName,
    url: config.nimara.wsBaseUrl,
  })

  // Dynamically load LLMAgent after the bot is initialized
  const agent = await createNeuriAgent(bot)
  await bot.loadPlugin(LLMAgent({ agent, nimaraClient }))
