...
```typescript
import { NIMARAAdapter, BrowserBaseMCPAdapter, TwitterService } from 'twitter-services'
...
async function startNIMARAModule() {
  ...
  const nimaraAdapter = new NIMARAAdapter(twitter, {
    url: process.env.NIMARA_URL,
    token: process.env.NIMARA_TOKEN
  })

  // Start adapter
  await nimaraAdapter.start()

  console.info('Twitter service running as NIMARA module')
}
```
