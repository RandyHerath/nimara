...
import { MarkdownRenderer } from '@proj-nimara/stage-ui/components'
  ...
  <div ref="chatHistoryRef" v-auto-animate flex="~ col" relative h-full w-full overflow-y-auto rounded-lg px="<sm:2" py="<sm:2">
    <div v-for="(message, index) in messages" :key="index" mb-2>
      ...
      <div v-if="message.role === 'assistant'" flex mr="12">
        ...
        >
          <div>
            <span text-xs text="primary-400/90 dark:primary-600/90" font-normal class="inline <sm:hidden">{{ t('stage.chat.message.character-name.nimara') }}</span>
          </div>
