...
export const useProvidersStore = defineStore('providers', () => {
  ...
  const envGeminiApiKey = import.meta.env.VITE_GEMINI_API_KEY as string | undefined
  const envGeminiBaseUrl = (import.meta.env.VITE_GEMINI_BASE_URL as string | undefined) ?? 'https://generativelanguage.googleapis.com/v1beta/openai/'

  if (envGeminiApiKey && !providerCredentials.value['google-generative-ai']) {
    providerCredentials.value['google-generative-ai'] = {
  ...
  const providerMetadata: Record<string, ProviderMetadata> = {
        ...
        validateProviderConfig: (config) => {
          if (!config.baseUrl) {
            return {
              ...
              reason: 'Base URL is required. This is likely a bug, report to developers on https://github.com/moeru-ai/nimara/issues.',
              valid: false,
            }
          }
        ...
        validateProviderConfig: (config) => {
          ...
          return fetch(`${(config.baseUrl as string).trim()}models`, { headers: (config.headers as HeadersInit) || undefined })
            ...
            .catch((err) => {
              return {
                errors: [err],
                reason: `Failed to reach Ollama server, error: ${String(err)} occurred.\n\nIf you are using Ollama locally, this is likely the CORS (Cross-Origin Resource Sharing) security issue, where you will need to set OLLAMA_ORIGINS=* or OLLAMA_ORIGINS=https://nimara.moeru.ai,${location.origin} environment variable before launching Ollama server to make this work.`,
                valid: false,
              }
            })
        },
        ...
        validateProviderConfig: (config) => {
          ...
          return fetch(`${(config.baseUrl as string).trim()}models`, { headers: (config.headers as HeadersInit) || undefined })
            ...
            .catch((err) => {
              return {
                errors: [err],
                reason: `Failed to reach Ollama server, error: ${String(err)} occurred.\n\nIf you are using Ollama locally, this is likely the CORS (Cross-Origin Resource Sharing) security issue, where you will need to set OLLAMA_ORIGINS=* or OLLAMA_ORIGINS=https://nimara.moeru.ai,http://localhost environment variable before launching Ollama server to make this work.`,
                valid: false,
              }
            })
        },
    ...
    'player2-speech': {
      ...
      createProvider: async config => createPlayer2((config.baseUrl as string).trim(), 'nimara'),
      capabilities: {
        listVoices: async (config) => {
