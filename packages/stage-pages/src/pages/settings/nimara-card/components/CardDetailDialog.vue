...
const selectedCard = computed<NimaraCard | undefined>(() => {
  if (!props.cardId)
    return undefined
...
const moduleSettings = computed(() => {
  if (!selectedCard.value || !selectedCard.value.extensions?.nimara?.modules) {
    return {
      consciousness: '',
      speech: '',
      voice: '',
  ...
  const nimaraExt = selectedCard.value.extensions.nimara.modules
  return {
    consciousness: nimaraExt.consciousness?.model || '',
    speech: nimaraExt.speech?.model || '',
    voice: nimaraExt.speech?.voice_id || '',
  }
})
