...
import type { ccv3 } from '@proj-nimara/ccc'

import { Alert } from '@proj-nimara/stage-ui/components'
import { useNimaraCardStore } from '@proj-nimara/stage-ui/stores/modules/nimara-card'
import { InputFile } from '@proj-nimara/ui'
import { Select } from '@proj-nimara/ui/components/form'
...
const { t } = useI18n()
const cardStore = useNimaraCardStore()
...
function getModuleShortName(id: string, module: 'consciousness' | 'voice') {
  const card = cards.value.get(id)
  if (!card || !card.extensions?.nimara?.modules)
    return 'default'

  const nimaraExt = card.extensions.nimara.modules

  if (module === 'consciousness') {
    return nimaraExt.consciousness?.model ? nimaraExt.consciousness.model.split('-').pop() || 'default' : 'default'
  }
  else if (module === 'voice') {
    return nimaraExt.speech?.voice_id || 'default'
  }
